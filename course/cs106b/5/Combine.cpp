...
#include "Combine.h"
using namespace std;
Vector<DataPoint> mergehelper(const Vector<DataPoint> &seq1, const Vector<DataPoint> &seq2)
{
    int i = 0, k=0;
    Vector<DataPoint> ans;
    // compare and merge
    while(i < seq1.size() && k<seq2.size()) {
           if(seq1[i].weight<seq2[k].weight){
                ans+=seq1[i];
                i++;
           }else{
                ans+=seq2[k];
...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    if(sequences.size()==2) return mergehelper(sequences[0],sequences[1]);
    int mid=sequences.size()/2;
    auto left = sequences.subList(0,mid);
    auto right = sequences.subList(mid,sequences.size()-mid);
    auto solved_l = combine(left);
    auto solved_r = combine(right);
    auto ans = mergehelper(solved_l,solved_r);
    return ans;
}

// Vector<DataPoint> MergeSort(const Vector<DataPoint>& seq1,
//                             const Vector<DataPoint>& seq2)
// {
//     Vector<DataPoint> ans ;
//     int p=0,q=0;
//     while(p<seq1.size()&&q<seq2.size())
//     {
//         if(seq1[p].weight<seq2[q].weight){ans.add(seq1[p]); p++;}
//         else {ans.add(seq2[q]); q++;}
//     }
//     while(p<seq1.size())
//     {
//         ans.add(seq1[p]);p++;
...
//     }
//     int mid = sequences.size()/2;
//     auto left = sequences.subList(0,mid); auto right = sequences.subList(mid,sequences.size()-mid);
//     auto solved_l = combine(left);
//     auto solved_r = combine(right);
//     auto ans = MergeSort(solved_l,solved_r);
//     return ans;
// }
