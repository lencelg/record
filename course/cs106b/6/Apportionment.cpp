...
double compute(int population,int allocatedseatnumber)
{
    allocatedseatnumber++;
    return population/sqrt(allocatedseatnumber*(allocatedseatnumber+1));
}
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    ...
    if(populations.size()>numSeats) {
        error("not enough to give each state a seat");
        return {};
        }
    int allocatedseatnumber=populations.size();
    HeapPQueue* hq=new HeapPQueue;
    Map<string, int> res=populations;
    for(string state:populations)
    {
        DataPoint temp={state,populations[state]/sqrt(2)*(-1)};
        hq->enqueue(temp);
    }
    for(string temp:res)
    {
    ...
    while(allocatedseatnumber<numSeats){
        allocatedseatnumber++;
        DataPoint tar=hq->dequeue();
        tar.weight=compute(populations[tar.name],res[tar.name]);
        tar.weight*=-1;
        res[tar.name]++;
        hq->enqueue(tar);
    }
