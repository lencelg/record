...
Nucleotide* toStrand(const string& str) {
    ...
    string copystr=str;
    char currentstr=copystr[0];
    res->next=nullptr,res->prev=nullptr,res->value=currentstr;
    auto ans=res;
    copystr=copystr.substr(1,copystr.size()-1);
    while(copystr!=""){
        currentstr=copystr[0];
        Nucleotide *temp=new Nucleotide;
        temp->value=currentstr;
        ans->next=temp;
        temp->prev=ans;
        temp->next=nullptr;
        ans=temp;
...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    /* TODO: Delete this comment and the next lines and implement this function. */
    if(dna==nullptr || target==nullptr) return nullptr;
    Nucleotide* temp=target;
    int cnt=0;
    while(temp!=nullptr){
        cnt++;
        temp=temp->next;
    ...
    int dnacnt=0;
    Nucleotide* dnacopy=dna;
    while(dnacopy!=nullptr){
        dnacnt++;
        dnacopy=dnacopy->next;
    }
    if(dnacnt<cnt) return nullptr;
    temp=target;
    while(dna!=nullptr){
        int cntcopy=cnt;
        if(dna->value==temp->value){
            cntcopy--;
            while(dna->next!=nullptr&&temp->next!=nullptr){
                dna=dna->next,temp=temp->next;
                if(dna->value!=temp->value) {
                    break;
                }
                cntcopy--;
            }
            if(cntcopy!=0&&dna->next==nullptr) return nullptr;
            if(cntcopy!=0){
                for (int i = 0; i < cnt - cntcopy - 1; ++i) {
                   dna=dna->prev;
                }
                temp=target;
                continue;
            }
            if(temp->value==dna->value){
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    ...
    if(dna==nullptr) return false;
    Nucleotide *first=findFirst(dna,target);
    if(first==nullptr) return false;
    Nucleotide *firstcopy=target;
    int cnt=0;
    ...
    firstcopy=first->prev;
    for (int i = 0; i < cnt; ++i) {
        Nucleotide *temp=first;
        first=first->next;
        delete temp;
    }
    firstcopy->next=first;
    if(first!=nullptr)  first->prev=firstcopy;
    return true;
}
...
PROVIDED_TEST("findFirst works when target is empty.") {
    ...
    EXPECT_EQUAL(findFirst(dna, nullptr), nullptr);
    deleteNucleotides(dna);
}
