 ...
 */
EncodingTreeNode* createnode(const char tar){
   auto newnode=new EncodingTreeNode;
   newnode->ch=tar;
   newnode->one=nullptr;
...
EncodingTreeNode* huffmanTreeFor(const string& str) {
    ...
    if(m.size()<2){
        error("the str should has at least two different characters");
        return nullptr;
    }
    PriorityQueue<pair<EncodingTreeNode *,int>> pq;
    for(auto &s:m.keys()){
        EncodingTreeNode* temp=createnode(s);
        pq.enqueue(pair(temp,m[s]),m[s]);
    }
    while(pq.size()>1){
        auto left=pq.dequeue();
        auto right=pq.dequeue();
        auto newnode=createnode(char(0));
        newnode->zero=left.first;
        newnode->one=right.first;
        int weight=left.second+right.second;
        pq.enqueue(pair(newnode,weight),weight);
    }
    EncodingTreeNode* ans=pq.peek().first;
    return ans;
}
 ...
 */
void makebook(Map<char,string>& book,EncodingTreeNode* tree,string s) {
   if(tree==nullptr)return;
   if(tree->one==nullptr&&tree->zero==nullptr)
   {
        book[tree->ch]=s;
   }
   makebook(book,tree->zero,s+"0");
   makebook(book,tree->one,s+"1");
}
 ...
 * or bits in them, etc.
 */
void decodeTreehelper(Queue<Bit>& bits, Queue<char>& leaves,EncodingTreeNode*& root) {
    if(bits.isEmpty()) return;
    if(bits.dequeue()==1){
        root->zero=new EncodingTreeNode;
        root->one=new EncodingTreeNode;
        decodeTreehelper(bits,leaves,root->zero);
        decodeTreehelper(bits,leaves,root->one);
    }else{
...
EncodingTreeNode* decodeTree(Queue<Bit>& bits, Queue<char>& leaves) {
    ...
    decodeTreehelper(bits,leaves,root);
    return root;
}
...
HuffmanResult compress(const string& text) {
    ...
    HuffmanResult res;
    auto s=huffmanTreeFor(text);
    res.messageBits=encodeText(text,s);
    Queue<char> leavestemp={};
    Queue<Bit> treebitstemp={};
    encodeTree(s,treebitstemp,leavestemp);
    res.treeBits=treebitstemp,res.treeLeaves=leavestemp;
    deleteTree(s);
    return res;
}
...
string decompress(HuffmanResult& file) {
    ...
    res=decodeText(file.messageBits,tree);
    deleteTree(tree);
    return res;
}
