...
bool RobinHoodHashTable::insert(const string& elem) {
    ...
    if(usedsize==slotsnums || this->contains(elem))	return false;
    int hashcode=hashFn(elem);
    Slot cur={elem,0};
    for (int i = 0;; ++i,hashcode=(hashcode+1)%slotsnums) {
       if(elems[hashcode].distance==EMPTY_SLOT){
            swap(elems[hashcode],cur);
            usedsize++;
            return true;
        }
       if (elems[hashcode].distance<cur.distance) {
            swap(elems[hashcode],cur);
        }
        cur.distance++;
    }
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    int hashcode=hashFn(elem);
    for (int i = 0; i < slotsnums; ++i,hashcode=(hashcode+1)%slotsnums) {
       if(elems[hashcode].value==elem && elems[hashcode].distance!=EMPTY_SLOT){
            return true;
       }
       if(i>elems[hashcode].distance) return false;
    }
    return false;
}
...
bool RobinHoodHashTable::remove(const string& elem) {
    /* TODO: Delete this comment and the next lines, then implement this function. */
    if(isEmpty() || !contains(elem)) return false;
    int currentpos= hashFn(elem) ;
    int findpos=currentpos;
    for (int i = 0; i < slotsnums; ++i,findpos=(findpos+1)%slotsnums) {
       if(elems[findpos].value==elem&&elems[findpos].distance!=EMPTY_SLOT){
            currentpos=findpos;
            break;
        }
       if(elems[findpos].value==elem&&elems[findpos].distance==EMPTY_SLOT) return false;
    }
    usedsize--;
    elems[currentpos].distance=EMPTY_SLOT;
    int next=(currentpos+1)%slotsnums;
    while(elems[next].distance!=0 && elems[next].distance!=EMPTY_SLOT) {
        elems[next].distance--;
        swap(elems[next],elems[currentpos]);
        currentpos=(currentpos+1)%slotsnums;
        next=(currentpos+1)%slotsnums;
    }
    return true;
    // if (isEmpty()) return false;
