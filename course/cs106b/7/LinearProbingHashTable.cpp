...
LinearProbingHashTable::LinearProbingHashTable(HashFunction<string> hashFn) {
    ...
    elems=new Slot[size];
    numofslots=myhash.numSlots(),numofelems=0;
...
bool LinearProbingHashTable::insert(const string& elem) {
    /* TODO: Delete this comment and the next lines, then implement this function. */
    if(numofelems==numofslots){
        return false;
    }else if (this->contains(elem)){
        return false;
    }else{
        int value = myhash(elem);
        while (elems[value].type == SlotType::FILLED) {
            value++;
            if (value >= myhash.numSlots()) value = 0;
    }
...
bool LinearProbingHashTable::contains(const string& elem) const {
    ...
    int hashcode=myhash(elem);
    for (int cnt = 0; cnt < numofslots && elems[hashcode].type!=SlotType::EMPTY; ++cnt,hashcode=(hashcode+1)%numofslots) {
       if(elems[hashcode].value==elem&&elems[hashcode].type!=SlotType::TOMBSTONE) return true;
    }
    return false;
}
...
bool LinearProbingHashTable::remove(const string& elem) {
    /* TODO: Delete this comment and the next lines, then implement this function. */
    int hashCode = myhash(elem);
    for(int i = hashCode,cnt = 0;
        elems[i].type!=SlotType::EMPTY && cnt!=numofslots;
        i = (i+1)%numofslots,cnt++)
    {
