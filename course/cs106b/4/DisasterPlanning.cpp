...
Set<string> coverCity(string city,
                      Set<string>& supplyLocations,
                      const Set<string> notCover,
                      const Map<string, Set<string>>& roadNetwork){
    auto newNotCover = notCover;
    newNotCover.remove(city);
    supplyLocations.add(city);
    for(auto & neighbor : roadNetwork[city]){
        if(newNotCover.contains(neighbor)){
            newNotCover.remove(neighbor);
        }
    }
    return newNotCover;
}

bool recCanBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
                               int numCities,
                               Set<string>& supplyLocations,
                               const Set<string> notCover){
    if(notCover.size() == 0) return true;
    if(numCities == 0) return false;
    string picked = notCover.first();
    // try to supply picked
    auto newNotCover = coverCity(picked,supplyLocations, notCover, roadNetwork);
    if(recCanBeMadeDisasterReady(roadNetwork, numCities - 1, supplyLocations, newNotCover)){
        return true;
    }else{
        supplyLocations.remove(picked);
    }
    for(auto neighbor : roadNetwork[picked]){
        auto newNotCover = coverCity(neighbor, supplyLocations, notCover, roadNetwork);
        if(recCanBeMadeDisasterReady(roadNetwork, numCities - 1, supplyLocations, newNotCover)){
            return true;
        }
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    for(auto & city : roadNetwork.keys()){
        if(! covered(city, roadNetwork, supplyLocations))
            notCover.add(city);
    }

    return recCanBeMadeDisasterReady(roadNetwork,
                                     numCities,
                                     supplyLocations,
                                     notCover);
}
