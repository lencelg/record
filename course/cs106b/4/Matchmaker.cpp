...
//         return true;
//     string choose=possibleLinks.firstKey();
//     bool flag=true;
//     // matching the current pair and check
//     for(auto possiblepair:possibleLinks[choose]){
//         for(auto s: matching ){
//             if(possiblepair==s.first()||possiblepair==s.second())
//                 flag=false;
//         }
//         if(flag){
//             // Pair another={choose,possiblepair};
//             // matching.add(another);
//             // possibleLinkd.remove(choose);
//             // possibleLinkd.remove(possiblepair);
//             // if(hasPerfectMatchingHelper(possibleLinkd,matching))
//             //     return true;
//             Pair New = {choose, possiblepair};
//             Set<Pair> plus = matching + New;
//             auto remaining = possibleLinks; //不要直接改变possibleLinks因为循环的下一次还要用到
//             remaining.remove(choose);remaining.remove(possiblepair);
//             if(hasPerfectMatching(remaining,plus))
//             {
//                 matching += plus;return true;
//             }
//         }
//     }
//     return false;
...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    ...
    if(possibleLinks.isEmpty())
        return true;
    // no match if the student number is odd
//     if(possibleLinks.size()/2!=0)
//         return false;
//     // no match if one student don't want to work in pair
//     for (string s:possibleLinks) {
//         if(possibleLinks.get(s).isEmpty()){
//             return false;
//     }
//     // general case
//     }
//     Map<string,Set<string>> copy=possibleLinks;
//     return hasPerfectMatchingHelper(copy,matching);
// }
    string person1=possibleLinks.firstKey();
    for(auto & person2:possibleLinks[person1]){
        bool flag=true;
        for(auto & s:matching){
            if(person2==s.first() or person2==s.second()){
                flag=false;
                break;
            }
        }
        if(flag){
                Pair another={person1,person2};
                auto remaining=possibleLinks;
                remaining.remove(person1);
                remaining.remove(person2);
                auto plus=matching + another;
                if(hasPerfectMatching(remaining,plus)){
                    matching+=plus;
                    return true;
                }
...
// i feel that i could not do this by my own , sucks!!! i am tired .
// we need to write the helper function to make it record and simple
...
//         auto copy=possibleLinks;
//         int max=0;
//         for(Pair )
//         for(string s: copy){
//             auto link=copy[s];
//             for(auto source : link.firstKey()){

//}
int totalWeight(const Map<string, Map<string, int>>& possibleLinks, Set<Pair> pairs){
    int sum = 0;
    for(const Pair & p : pairs){
        sum += possibleLinks[p.first()][p.second()];
    }
    return sum;
}

Map<string, Map<string, int>> removeNode(const Map<string, Map<string, int>> map, const string & toRemove){
    Map<string, Map<string, int>> newMap = map;
    for(auto & node : map){
        if(node == toRemove){
            newMap.remove(toRemove);
            continue;
        }
        for(auto & partner : map[node]){
            if(partner == toRemove){
                newMap[node].remove(partner);
            }
        }
    }
    // remove node if having no edge
    Map<string, Map<string, int>> res = newMap;
    for(auto & node : newMap){
        if(newMap[node].size() == 0) res.remove(node);
    }
    return res;
}

Set<Pair> recmaximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    // nodes in possibleLinks are all having positive edge
    // base case
    // if no positive edge for the map. done
    if(possibleLinks.size() == 0){
        return {};
    }
    // recursive
    // pick a node and try all possible edge, choose the pairs with highest weight
    auto picked = possibleLinks.firstKey();
    auto newPossibleLinks = removeNode(possibleLinks, picked); // remove node
    Set<Pair> bestSet =  recmaximumWeightMatching(newPossibleLinks); // choose no edge on picked
    int maximunWeight = totalWeight(possibleLinks, bestSet);
    for(auto & partner : possibleLinks[picked]){
        // remove partner
        auto set = recmaximumWeightMatching(removeNode(newPossibleLinks, partner)) + Pair(picked, partner);
        int weight = totalWeight(possibleLinks ,set);
...
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    Map<string, Map<string, int>> newPossibleLinks = possibleLinks;
    for(auto & node : possibleLinks){
        Map<string, int> new_edges = possibleLinks[node];
        // remove edge with negative or zero weigth
        for(auto & partner : possibleLinks[node]){
            if(possibleLinks[node][partner] <= 0){
                new_edges.remove(partner);
            }
        }
        newPossibleLinks[node] = new_edges;
        // remove node without valid edge
        if(new_edges.size() == 0) newPossibleLinks.remove(node);
    }

    return recmaximumWeightMatching(newPossibleLinks);
}
