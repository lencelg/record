...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    Map<string,double> a{};
    if(kGramLength<0){
        error("the k-gram length is negative");
        return {};
    }else if (kGramLength==0){
        error("the k-gram length is zero");
        return {};
    }else{
        for (int i=0;i+kGramLength<=str.length();++i){
            string now=str.substr(i,kGramLength);
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    for(string now: input){
        sum=sum+input[now]*input[now];
    }
    if(sum==0){
        error("there is nonzero number");
        return {};
    }else{
        sum=sqrt(sum);
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    if(numToKeep<0){
        ...
        for(string s:source){
            q.enqueue(s,source[s]);
        }
        Map<string,double> chosen;
        while (q.size()>numToKeep) {
            q.dequeue();
        }
        while (!q.isEmpty()) {
            string s=q.peek();
            chosen.put(s,source[s]);
            q.dequeue();
        }
        return chosen;
    }
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
     ...
     */
    double s=0;
    for(string chosen : lhs){
        if (rhs.containsKey(chosen)){
            s+=lhs[chosen]*rhs[chosen];
        }
    }
    return s ;
}
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    if (corpora.isEmpty()){
        ...
        double evaluate=0;
        for(Corpus temp:corpora){
            double compare=cosineSimilarityOf(textProfile,temp.profile);
            if	(compare>evaluate){
                evaluate=compare;
                chosenlanguage=temp.name;
            }
        }
