...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    Grid<bool> answer=Grid<bool>(terrain.numRows(),terrain.numCols());
    answer.fill(false);
    Queue<GridLocation> q;
    for(GridLocation chosen:sources){
        if	(terrain.get(chosen.row,chosen.col)<=height){
    ...
    while(!q.isEmpty()){
        ...
        if(terrain.inBounds(temp.row+1,temp.col)&&answer[temp.row+1][temp.col]==false&&terrain.get(temp.row+1,temp.col)<=height){
            answer[temp.row+1][temp.col]=true;
            GridLocation chosen=temp;
            chosen.row++;
            q.enqueue(chosen);
        }
        if(terrain.inBounds(temp.row-1,temp.col)&&answer[temp.row-1][temp.col]==false&&terrain.get(temp.row-1,temp.col)<=height){
        ...
        if(terrain.inBounds(temp.row,temp.col+1)&&answer[temp.row][temp.col+1]==false&&terrain.get(temp.row,temp.col+1)<=height){
            answer[temp.row][temp.col+1]=true;
            GridLocation chosen=temp;
            chosen.col++;
            q.enqueue(chosen);
        }
