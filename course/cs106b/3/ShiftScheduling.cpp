 ...
 *  it is a hard task and i refer to the answer first
 *  compare the time , choose , unchoose , compare the value , make decision
 *
 *
 *  after passing all the test , if there is a solution that use the highest per hour value and decision the shift round
 *  thinking it for a moment........
 *  and my answer is that it is not workable
 *  the stratage is choose the best in each time and it is not the best case but the most close to best case
 *  As it is shown in the docs that the example case tells that it is wrong!!!
 */
Set<Shift> highestValueScheduleForHelper(const Set<Shift>& shifts, int maxHours,const Set<Shift>& current){
    ...
    if(shifts.size()==0){
        ...
        // check the time confliction
        int currenttime=0;
        Shift choose=shifts.first();
        auto ifchoose =current+choose;
        bool flag=true;
        for(Shift s:ifchoose)
            currenttime+=lengthOf(s);
        if(currenttime>maxHours)
            flag=false;
        // choose one and unchoose it
        for(auto s:current){
            if(overlapsWith(choose,s)){
                flag=false;
                break;
            }
        }
        auto remaining =shifts-choose;
        // not confilct and not time exceed
        if(flag){
            auto a= highestValueScheduleForHelper(remaining,maxHours,ifchoose);
            auto b= highestValueScheduleForHelper(remaining,maxHours,current);
            double x=0;
            double y=0;
            for(auto s:a){x+=valueOf(s);}
            for(auto s:b){y+=valueOf(s);}
            return x>=y?a:b;
        }else{
            return highestValueScheduleForHelper(remaining,maxHours,current);
        }
...
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    ...
    if(maxHours<0){
        error("the maxHour is negative!");
    }else if (maxHours==0){
        return {};
    }else {
        return highestValueScheduleForHelper(shifts,maxHours,{});
    }
}
