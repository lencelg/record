...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while len(temp_place.bees)==0:
            temp_place=temp_place.entrance
        if temp_place.is_hive: return None
        return random_bee(temp_place.bees)
        """ 
        # END Problem 3 and 4
        temp_place=self.place
        step=0
        while step<self.lower_bound:
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        return False if self.ant_contained!=None or other.is_container else True
        # END Problem 8a
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while temp_place!=None:
            ants=temp_place.ant
            if isinstance(ants,ContainerAnt):
               temp_place.ant.double()
               if temp_place.ant.ant_contained:
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        if target is not None:
            target.turns=5
            target.action=bee_action.__get__(target,Bee)
        # END Problem EC 1
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while temp_place is not None:
            ant=temp_place.ant
            if not temp_place.is_hive:
                if ant is not self and ant is not None:
                    res[ant]=cur_distance
                for bee in temp_place.bees:
                    res[bee]=cur_distance
            cur_distance+=1
            temp_place=temp_place.entrance
        return res
        # END Problem EC 4
