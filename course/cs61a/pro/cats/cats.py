...
def about(subject):
    ...
    def inner(paragraph):
        for cur in subject:
                if cur in split(lower(remove_punctuation(paragraph))) : return True 
        return False
    return inner
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    if len(source_words)==0 or len(typed_words)==0: return 0.0 
    cnt=0
    s=min(len(source_words),len(typed_words))
    for i in range(s):
        if source_words[i]==typed_words[i]: cnt+=1
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    cur=min(word_list,key=lambda i : diff_function(typed_word,i,limit))
    min_diff=diff_function(typed_word,cur,limit)
    if min_diff>limit:
        return typed_word
    return cur 
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    # recursion was required in this question
    def inner(string1,string2,cnt):
        if cnt>limit : return cnt
        if string1=="" or string2=="" : return cnt
        if string1[0]!=string2[0]: cnt+=1
        return inner(string1[1:],string2[1:],cnt)
    result=inner(typed,source,0)+abs(len(typed)-len(source))
    return result
    # END PROBLEM 6
...
def minimum_mewtations(typed, source, limit): 
    ...
    3
    """
    # first version code , not work for the Extra challenge(Phase 4)
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in tpp:
        cur=[]
        for j in range(len(i)-1): cur.append(i[j+1]-i[j])            
        times.append(cur) 
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    "*** YOUR CODE HERE ***"
    res=[]
    for i in player_indices: res.append([])
    for i in word_indices:
        lowest=get_time(times,0,i)
        cur=0
        for j in player_indices:
            if lowest>get_time(times,j,i): 
                lowest=get_time(times,j,i)
                cur=j
        res[cur].append(words[i])
    return res
    # END PROBLEM 10
