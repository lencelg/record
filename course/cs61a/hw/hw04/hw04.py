...
def balanced(m):
    ...
    leftarm=left(m)
    rightarm=right(m)
    leftweight=length(leftarm)*total_mass(end(leftarm))
    rightweight=length(rightarm)*total_mass(end(rightarm))
    return leftweight==rightweight and balanced(end(leftarm)) and balanced(end(rightarm)) 
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        ...
        for branche in branches(t):
            res.append(max_path_sum(branche))
    return max(res)+label(t)
